...
class OwnerCog(commands.Cog):
    ...
    async def owner(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            await self.transport.send(ctx.channel, "👑 **Sovereign Authority recognized.** Options: `!owner skip [hours]`, `!owner start forge [bid]`, `!owner tick`, `!owner setup`")

    @owner.group(name="start", invoke_without_command=True)
    async def owner_start(self, ctx):
        """Start systems regardless of requirements."""
        if ctx.invoked_subcommand is None:
            await self.transport.send(ctx.channel, "🏮 **Please specify what to start (e.g., !owner start forge [id])**")
    ...
    async def owner_start_forge(self, ctx, bid: str):
        ...
        if success:
            await self.transport.send(ctx.channel, f"⚡ **Sovereign Override:** {message}")
        else:
            await self.transport.send(ctx.channel, f"❌ **Override Failed:** {message}")
    ...
    async def owner_skip(self, ctx, hours: float):
        ...
        if slayer_skipped:
            await self.transport.send(ctx.channel, f"⏳ **Time Warped:** Advanced the hunt by **{hours} hours**.")
    ...
    async def owner_tick(self, ctx):
        ...
        await self.transport.send(ctx.channel, "⚡ **Invoking the Sovereign Heartbeat...**")
