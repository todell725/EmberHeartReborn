...
into the runtime state/SIDE_QUESTS_DB.json for the game engine.
"""
...
def run():
    ...
    compiled_quests.sort(key=lambda x: x.get("id", ""))

    logger.info(f"Validation successful. Compiling {len(compiled_quests)} quests...")
    
    # Write to runtime state as a flat array (as expected by the legacy bot)
